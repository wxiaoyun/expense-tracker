...
jest.mock("@/libs/app-runtime", () => ({
  ...
  reinitializeAppRuntime: (...args: unknown[]) =>
    mockReinitializeRuntime(...args),
  waitForLaunchTemplateProcessing: (...args: unknown[]) =>
    mockWaitForLaunchProcessing(...args),
}));
...
jest.mock("@expo/ui", () => {
  ...
  const Text = ({
    ...
    React.createElement(
      RNText,
      { testID, style: textStyle, onPress },
      children,
    );
...
describe("Settings database runtime orchestration", () => {
  ...
  it("registers daily auto backup when enabled", async () => {
    ...
    await waitFor(() =>
      expect(mockSetAutoBackup).toHaveBeenCalledWith("daily"),
    );
  });
  ...
  it("waits, imports, reinitializes imported state, then returns to normal tabs", async () => {
    ...
    const confirmation = alert.mock.calls.find(
      ([title]) => title === "Import database",
    )!;
    await act(async () => confirmation[2]![1].onPress?.());

    await waitFor(() =>
      expect(mockReinitializeRuntime).toHaveBeenCalledWith({
    ...
    expect(mockImportDatabase.mock.invocationCallOrder[0]).toBeLessThan(
      mockReinitializeRuntime.mock.invocationCallOrder[0],
    );
    expect(mockReinitializeRuntime.mock.invocationCallOrder[0]).toBeLessThan(
  ...
  it("imports a DBS CSV, refreshes transaction queries, and reports counts", async () => {
    ...
    mockGetDocument.mockResolvedValueOnce({
      canceled: false,
      assets: [{ uri: "file:///dbs.csv" }],
    });
    mockReadAsString.mockResolvedValueOnce("csv-text");
    mockParseDbsCsv.mockReturnValueOnce({
      rows: [{ id: "a" }, { id: "b" }],
      unreadable: 1,
    });
  ...
  it("waits, resets, reinitializes defaults and one-shot state, then opens migration", async () => {
    ...
    expect(mockResetAllData.mock.invocationCallOrder[0]).toBeLessThan(
      mockReinitializeRuntime.mock.invocationCallOrder[0],
    );
    expect(mockReinitializeRuntime.mock.invocationCallOrder[0]).toBeLessThan(
      mockReplace.mock.invocationCallOrder[0],
