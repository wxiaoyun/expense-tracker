...
jest.mock("react-native", () => {
  ...
  const MockTextInput = React.forwardRef(
    (props: { accessibilityLabel?: string }, ref: React.Ref<unknown>) => {
...
jest.mock("@/db/template", () => ({
  ...
  previewTemplateBackfill: (...args: unknown[]) =>
    mockPreviewTemplateBackfill(...args),
...
const template = (
  overrides: Partial<TransactionTemplate> = {},
): TransactionTemplate => {
  const base = draft();
  return {
    ...base,
...
describe("TemplateEditDrawer", () => {
  beforeEach(() => {
    ...
    mockCreateTemplate.mockImplementation(async (value: TemplateDraft) =>
      template({
        ...value,
        id: "created-1",
    ...
    mockUpdateTemplate.mockImplementation(
      async (id: string, value: TemplateDraft) =>
        template({
          ...value,
          id,
          verified: value.verified ? 1 : 0,
  ...
  it("selecting a suggestion populates fields without saving", async () => {
    ...
    await fireEvent.press(
      screen.getByRole("button", { name: "Use suggestion Train pass" }),
    );

    expect(screen.getByLabelText("Template name")).toHaveProp(
      "value",
      "Train pass",
    );
    expect(screen.getByLabelText("Template amount")).toHaveProp("value", "12");
    expect(screen.getByLabelText("Template description")).toHaveProp(
      "value",
      "Train pass",
    );
  ...
  it("edit mode loads an existing template and does not autofocus", async () => {
    ...
    mockGetTemplate.mockResolvedValue(
      template({ name: "Edited coffee", amount: 8 }),
    );
  ...
  it("source transaction populates name and transaction values, omits date, and hides suggestions", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Transaction type: Income" }).props
        .accessibilityState,
  ...
  it("keeps invalid cron preview safe and only shows validation after Save", async () => {
    ...
    await fireEvent(
      screen.getByRole("switch", { name: "Repeat automatically" }),
      "valueChange",
      true,
    );
    await fireEvent.changeText(
  ...
  it("shows duplicate normalized name inline with an actionable suffix", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText("Template name already exists")).toBeTruthy(),
    );
    expect(
      screen.getByRole("button", { name: "Use suggested name Coffee 2" }),
    ).toBeTruthy();
    await fireEvent.press(
      screen.getByRole("button", { name: "Use suggested name Coffee 2" }),
    );
  ...
  it("Repeat automatically reveals presets, raw cron, start date, and the next three dates", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Recurrence preset Daily" }),
    ).toBeTruthy();
    expect(screen.getByLabelText("Cron expression")).toBeTruthy();
    expect(screen.getByTestId("template-start-date")).toBeTruthy();
    await waitFor(() =>
      expect(screen.getAllByTestId("next-occurrence")).toHaveLength(3),
    );
  });
  ...
  it("requires confirmation before disabling repetition on a scheduled template", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("switch", { name: "Repeat automatically" }).props
          .value,
      ).toBe(true),
    );
    ...
    expect(
      screen.getByRole("switch", { name: "Repeat automatically" }).props.value,
    ).toBe(true);
  ...
  it("updates other fields while activating an inactive edited schedule", async () => {
    ...
    await fireEvent(
      screen.getByRole("switch", { name: "Schedule active" }),
      "valueChange",
      true,
    );
    await fireEvent.press(screen.getByRole("button", { name: "Save" }));
  ...
  it("keeps an active saved template and explains launch retry when backfill fails", async () => {
    ...
    await waitFor(() =>
      screen.getByText("Saving will create 2 past transactions."),
    );
    await fireEvent.press(screen.getByRole("button", { name: "Save" }));
    await waitFor(() =>
      expect(alertSpy).toHaveBeenCalledWith(
    ...
    const confirmation = alertSpy.mock.calls.find(
      (call) => call[0] === "Create 2 past transactions?",
    )!;
  ...
  it("confirms and completes explicit backfill for an inactive schedule", async () => {
    ...
    await waitFor(() =>
      expect(mockBackfillTemplate).toHaveBeenCalledWith(
        "created-1",
        expect.any(Number),
      ),
    );
    expect(mockCreateTemplate).toHaveBeenCalledWith(
      expect.objectContaining({ scheduleActive: false }),
    );
  ...
  it("previews and confirms the exact past-start backfill count before save", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText("Saving will create 7 past transactions."),
      ).toBeTruthy(),
    );
    await fireEvent.press(screen.getByRole("button", { name: "Save" }));
    await waitFor(() =>
    ...
    const submissionPreviewCall = mockPreviewTemplateBackfill.mock.calls.find(
      (call) => call.length === 2,
    );
    expect(submissionPreviewCall).toEqual([
      expect.any(Object),
      expect.any(Number),
    ]);
    expect(mockBackfillTemplate).toHaveBeenCalledWith(
      "created-1",
      submissionPreviewCall![1],
    );
  });
