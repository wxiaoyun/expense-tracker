...
const getActiveTransactionConditions = ({
  ...
  const whereConditions = [
    ...
    between(transactions.transactionDate, startTs, endTs),
  ];
...
export const getTransaction = async (
  ...
  try {
    ...
    console.error("[transactions.get][stage=query] transaction fetch failed", {
      id,
      error: String(error),
    });
    throw error;
...
export const listTransactions = async (query?: {
  ...
  const { startTs, endTs, categories, verified } =
    getTransactionWhereContext(query);
  ...
  const orderDirection =
    orderDir === "DESC" ? desc(orderColumn) : asc(orderColumn);
  const whereConditions = getActiveTransactionConditions({
    startTs,
    endTs,
    categories,
    verified,
  ...
  try {
    const row = await db
      .select({
        ...
        income: sum(
          sql<number>`CASE WHEN ${transactions.amount} > 0 THEN ${transactions.amount} ELSE 0 END`,
        ),
        expense: sum(
          sql<number>`CASE WHEN ${transactions.amount} < 0 THEN ${transactions.amount} ELSE 0 END`,
        ),
    ...
    console.error(
      "[transactions.summary][stage=query] transaction summary failed",
      {
        startTs,
        endTs,
        category_count: categories.length,
...
export const createTransaction = async (
  ...
  const now = Date.now();
  const id = createId();
  console.info("[transactions.create][stage=insert] creating transaction", {
    id,
  });
  ...
  try {
    ...
    console.error(
      "[transactions.create][stage=insert] transaction create failed",
...
export const updateTransaction = async (
  ...
  const now = Date.now();
  console.info("[transactions.update][stage=update] updating transaction", {
    id: transaction.id,
  });
  ...
  try {
    ...
    console.error(
      "[transactions.update][stage=update] transaction update failed",
      { id: transaction.id, error: String(error) },
    );
    throw error;
  }
};
...
export const softDeleteTransaction = async (
  ...
  console.info(
    "[transactions.delete][stage=soft_delete] deleting transaction",
    { id, deletedAt },
  );
  try {
    const result = await db
    ...
    console.error(
      "[transactions.delete][stage=soft_delete] transaction delete failed",
      { id, error: String(error) },
    );
    throw error;
  }
...
export const listCategoriesByUsage = async (): Promise<Category[]> => {
  console.info(
    "[transactions.categories_by_usage][stage=query] listing categories sorted by usage",
    {},
  );
  try {
    ...
    const usageByName = new Map(
      usageRows.map((row) => [row.category, row.usage]),
    );
    const knownNames = new Set(presetRows.map((row) => row.name));
    const customRows: Category[] = usageRows
      .filter(
        (row) => row.category.trim() !== "" && !knownNames.has(row.category),
      )
      .map((row) => ({
    ...
    const rows = [...presetRows, ...customRows].sort((a, b) => {
      const usageDiff =
        (usageByName.get(b.name) ?? 0) - (usageByName.get(a.name) ?? 0);
      if (usageDiff !== 0) return usageDiff;
      if (a.sort_order !== b.sort_order) return a.sort_order - b.sort_order;
      return a.name.localeCompare(b.name);
    });
    console.info(
      "[transactions.categories_by_usage][stage=query] categories listed",
      {
    ...
    console.error(
      "[transactions.categories_by_usage][stage=query] category usage listing failed",
      { error: String(error) },
    );
    throw error;
  }
...
export const listCategories = async (): Promise<string[]> => {
  ...
  console.info(
    "[transactions.categories][stage=query] listing active transaction categories",
    queryContext,
  );
  try {
    const rows = await db
    ...
    console.error(
      "[transactions.categories][stage=query] category list failed",
      { ...queryContext, error: String(error) },
    );
    throw error;
  }
};
...
export const clearTransactions = async (): Promise<void> => {
  ...
  console.info(
    "[transactions.clear][stage=physical_delete] clearing all transactions",
    queryContext,
  );
  try {
    ...
    console.error(
      "[transactions.clear][stage=physical_delete] transaction clear failed",
      { ...queryContext, error: String(error) },
    );
    throw error;
  }
...
export const batchCreateTransactions = async (
  ...
  console.info(
    "[transactions.batch_create][stage=insert] creating transactions",
    { count: values.length },
  );
  ...
  try {
    ...
    console.error(
      "[transactions.batch_create][stage=insert] transaction batch create failed",
      { count: values.length, error: String(error) },
    );
    throw error;
  }
};

export const summarizeByCategory = async (query?: {
  start?: Date;
  end?: Date;
  categories?: string[];
}): Promise<
  | { category: string; balance: number; income: number; expense: number }[]
  | null
> => {
  ...
  console.info(
    "[transactions.summary_by_category][stage=query] summarizing transactions by category",
    {
      startTs,
      endTs,
      category_count: categories.length,
  ...
  try {
    ...
    console.error(
      "[transactions.summary_by_category][stage=query] category summary failed",
      {
        startTs,
        endTs,
        category_count: categories.length,
...
export const summarizeByMonth = async (query?: {
  start?: Date;
  end?: Date;
  categories?: string[];
}) => {
  ...
  console.info(
    "[transactions.summary_by_month][stage=query] summarizing transactions by month",
    {
      startTs,
      endTs,
      category_count: categories.length,
  ...
  try {
    const rows = await db
      ...
      .where(and(...getActiveTransactionConditions(context)))
      .groupBy(
        sql`strftime('%Y-%m', ${transactions.transactionDate} / 1000, 'unixepoch')`,
      )
      .all();
    return rows
      .map((row) => ({ month: row.month, expense: Number(row.expense ?? 0) }))
      .sort((a, b) => a.month.localeCompare(b.month));
  } catch (error) {
    console.error(
      "[transactions.summary_by_month][stage=query] monthly summary failed",
      {
        startTs,
        endTs,
        category_count: categories.length,
...
export const summarizeCashFlowByPeriod = async (
  ...
  console.info(
    "[transactions.summary_by_period][stage=query] summarizing cash flow by period",
    {
      startTs,
      endTs,
      category_count: categories.length,
  ...
  try {
    ...
    console.error(
      "[transactions.summary_by_period][stage=query] cash flow summary failed",
      {
        startTs,
        endTs,
        category_count: categories.length,
...
export const listTemplateSuggestionRows = async (
  startDate: number,
): Promise<HistoricalTransactionInput[]> => {
  console.info(
    "[transactions.template_suggestions][stage=query] listing historical suggestion rows",
    { startDate },
  );
  try {
    return await db
    ...
    console.error(
      "[transactions.template_suggestions][stage=query] historical suggestion rows failed",
      { startDate, error: String(error) },
    );
    throw error;
  }
};
...
export const setVerification = async (
  ...
  console.info(
    "[transactions.verify][stage=update] updating transaction verification",
    { id, verified },
  );
  try {
    const result = await db
    ...
    console.error(
      "[transactions.verify][stage=update] transaction verification failed",
      { id, verified, error: String(error) },
    );
    throw error;
  }
};
