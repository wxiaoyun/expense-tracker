import { and, asc, between, desc, eq, inArray, sql, sum } from "drizzle-orm";
...
export const listTransactions = async (query?: {
  ...
  const getOrderColumn = (key: keyof Transaction) => {
    switch (key) {
  ...
  if (verified !== undefined) {
    whereConditions.push(eq(transactions.verified, verified));
  }
...
export const createTransaction = async (
  ...
  const result = await db
    ...
    .values({
      ...transaction,
      verified: (transaction as any).verified ?? 0,
      createdAt: now,
      updatedAt: now,
    })
    .run();
