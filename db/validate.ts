...
export const validateDatabase = async (
  ...
  try {
    console.info(
      "[DB][validateDatabase] Starting validation for:",
      databaseName,
    );
    ...
    console.info(
      "[DB][validateDatabase] SQLite version:",
      versionResult.version,
    );

    // Validate table structure
    for (const [tableName, expectedStructure] of Object.entries(
      EXPECTED_TABLES,
    )) {
      ...
      if (!isValid) {
        console.error(
          `[DB][validateDatabase] Table validation failed: ${tableName}`,
        );
        return false;
      }
    }
    ...
    if (db) {
      try {
        db.closeSync();
        console.info(
          "[DB][validateDatabase] Validation database connection closed",
        );
      } catch (closeError) {
        console.warn(
          "[DB][validateDatabase] Error closing validation database:",
          closeError,
        );
      }
    }
...
const validateTable = async (
  ...
  try {
    ...
    const columns = await db.all(sql.raw(`PRAGMA table_info(${tableName})`));
    ...
    for (const expectedColumn of expectedStructure.columns) {
      if (!columnNames.includes(expectedColumn)) {
        console.error(
          `[DB][validateTable] Missing column '${expectedColumn}' in table '${tableName}'`,
        );
        return false;
      }
    }
    ...
    for (const expectedIndex of expectedStructure.indexes) {
      if (!indexNames.includes(expectedIndex)) {
        console.warn(
          `[DB][validateTable] Missing index '${expectedIndex}' in table '${tableName}' (non-critical)`,
        );
    ...
    console.error(
      `[DB][validateTable] Error validating table '${tableName}':`,
      error,
    );
    return false;
  }
...
const performIntegrityChecks = async (
  ...
  try {
    ...
    const integrityResult = await db.get<{ integrity_check: string }>(
      sql`PRAGMA integrity_check`,
    );

    if (integrityResult && integrityResult.integrity_check !== "ok") {
      console.error(
        "[DB][performIntegrityChecks] Database integrity check failed:",
        integrityResult,
      );
      return false;
    }
    ...
    if (foreignKeyResult) {
      console.error(
        "[DB][performIntegrityChecks] Foreign key constraint violations found:",
        foreignKeyResult,
      );
      return false;
    }
    ...
    try {
      ...
      if (categorySample) {
        ...
        if (
          typeof categorySample.id !== "string" ||
          ...
          console.error(
            "[DB][performIntegrityChecks] Invalid data types in categories table",
          );
          return false;
        }
      }
      ...
      if (transactionSample) {
        // Validate basic data types
        if (
          typeof transactionSample.id !== "string" ||
          ...
          console.error(
            "[DB][performIntegrityChecks] Invalid data types in transactions table",
          );
          return false;
        }
      }
      ...
      if (templateSample) {
        if (
          typeof templateSample.id !== "string" ||
          ...
          console.error(
            "[DB][performIntegrityChecks] Invalid data types in transaction_templates table",
          );
          return false;
        }
      }
      ...
      if (settingsSample) {
        if (
          typeof settingsSample.key !== "string" ||
          typeof settingsSample.value !== "string"
        ) {
          console.error(
            "[DB][performIntegrityChecks] Invalid data types in settings table",
          );
          return false;
        }
      }
      ...
      console.info(
        "[DB][performIntegrityChecks] Tables appear to be empty, skipping data type validation",
      );
    }
    ...
    console.error(
      "[DB][performIntegrityChecks] Error during integrity checks:",
      error,
    );
    return false;
  }
 ...
 */
export const quickValidateDatabase = async (
  databasePath: string,
): Promise<boolean> => {
  let db: SQLite.SQLiteDatabase | null = null;
  let stage = "open";
  ...
  try {
    ...
    stage = "inspect";
    console.info(
      "[db.quick_validate][stage=inspect] inspecting database requirements",
    );
    const integrity = db.getFirstSync<{ integrity_check: string }>(
    ...
    const transactionColumnsOk =
      transactionColumns.length ===
        EXPECTED_TABLES.transactions.columns.length &&
      EXPECTED_TABLES.transactions.columns.every(
        (name, index) => transactionColumns[index] === name,
      );
    const templateColumnsOk =
      templateColumns.length ===
        EXPECTED_TABLES.transaction_templates.columns.length &&
      EXPECTED_TABLES.transaction_templates.columns.every(
        (name, index) => templateColumns[index] === name,
      );
    ...
    if (db) {
      try {
        db.closeSync();
        console.info(
          "[db.quick_validate][stage=close] database connection closed",
        );
      } catch (error) {
        console.error(
 ...
 */
export const lenientValidateDatabase = async (
  databasePath: string,
): Promise<boolean> => {
  let db: SQLite.SQLiteDatabase | null = null;
  ...
  try {
    console.info(
      "[DB][lenientValidateDatabase] Opening database:",
      databasePath,
    );
    db = SQLite.openDatabaseSync(databasePath);
    ...
    try {
      ...
      console.info(
        "[DB][lenientValidateDatabase] Basic connectivity test:",
        testQuery,
      );

      // Check if it has any tables at all
      const tableCount = db.getFirstSync(
    ...
    if (db) {
      try {
        db.closeSync();
        console.info(
          "[DB][lenientValidateDatabase] Database connection closed",
        );
      } catch (closeError) {
        console.warn(
          "[DB][lenientValidateDatabase] Error closing database:",
          closeError,
        );
      }
    }
 ...
 */
export const rawSqliteValidateDatabase = async (
  databaseName: string,
): Promise<boolean> => {
  let db: SQLite.SQLiteDatabase | null = null;
  ...
  try {
    console.info(
      "[DB][rawSqliteValidateDatabase] Opening database:",
      databaseName,
    );
    db = SQLite.openDatabaseSync(databaseName);
    ...
    try {
      ...
      console.info(
        "[DB][rawSqliteValidateDatabase] Basic connectivity:",
        testResult,
      );
    } catch (connectError) {
      console.error(
        "[DB][rawSqliteValidateDatabase] Basic connectivity failed:",
        connectError,
      );
      return false;
    }
    ...
    try {
      ...
      const hasTransactionTemplates = tableNames.includes(
        "transaction_templates",
      );
      const hasSettings = tableNames.includes("settings");

      console.info(
        "[DB][rawSqliteValidateDatabase] Has categories table:",
        hasCategories,
      );
      console.info(
        "[DB][rawSqliteValidateDatabase] Has transactions table:",
        hasTransactions,
      );
      console.info(
        "[DB][rawSqliteValidateDatabase] Has transaction_templates table:",
        hasTransactionTemplates,
      );
      console.info(
        "[DB][rawSqliteValidateDatabase] Has settings table:",
        hasSettings,
      );
      ...
      if (
        ...
        console.info(
          "[DB][rawSqliteValidateDatabase] All required tables found - validation passed",
        );
        return true;
      }
      ...
      if (tableNames.length > 0) {
        console.info(
          "[DB][rawSqliteValidateDatabase] Database has tables but not the expected schema",
        );
        return true; // Allow import of any valid SQLite database
      }

      console.warn(
        "[DB][rawSqliteValidateDatabase] No tables found in database",
      );
      return false;
    } catch (queryError) {
      console.error(
        "[DB][rawSqliteValidateDatabase] Table query failed:",
        queryError,
      );
      return false;
    }
    ...
    if (db) {
      try {
        db.closeSync();
        console.info(
          "[DB][rawSqliteValidateDatabase] Database connection closed",
        );
      } catch (closeError) {
        console.warn(
          "[DB][rawSqliteValidateDatabase] Error closing database:",
          closeError,
        );
      }
    }
