...
export const suggestionLookbackStart = (
...
const isReusableAmountInvalid = (amount: number | null) =>
...
const buildKey = (
  ...
  [
    normalizeTemplateText(description),
    normalizeTemplateText(category),
    transactionType,
  ].join("|");
...
const buildActiveTemplateKey = (template: TemplateDraft) => {
  ...
  return buildKey(
    description,
    trimText(template.category) || "Other",
...
export const buildTemplateSuggestions = (
  ...
  for (const row of rows) {
    ...
    const amountStats = current.amountCounts.get(amount) ?? {
      count: 0,
      mostRecentAt: 0,
    };
    amountStats.count += 1;
    amountStats.mostRecentAt = Math.max(
      amountStats.mostRecentAt,
      row.transactionDate,
    );
  ...
  const suggestions = eligibleGroups.map<TemplateSuggestion>((group) => {
    ...
    for (const [amount, stats] of group.amountCounts) {
      if (
        ...
        (stats.count === selectedStats.count &&
          stats.mostRecentAt > selectedStats.mostRecentAt) ||
        (stats.count === selectedStats.count &&
          stats.mostRecentAt === selectedStats.mostRecentAt &&
  ...
  return suggestions
    .sort((left, right) => {
      ...
      if (right.mostRecentAt !== left.mostRecentAt)
        return right.mostRecentAt - left.mostRecentAt;
...
export const nextAvailableTemplateName = (
  base: string,
  activeNames: string[],
): string => {
  ...
  const taken = new Set(
    active.map(({ trimmed: activeName }) => normalizeTemplateText(activeName)),
  );
