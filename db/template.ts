...
const failure = (
  ...
  console.error(`[templates.${operation}][stage=${stage}] failed`, {
    ...details,
    error: String(error),
  });
};
...
const safeDraftDetails = (draft: TemplateDraft) => ({
  ...
  schedule_active:
    Boolean(draft.recurrenceValue?.trim()) && draft.scheduleActive,
...
const activeNameExists = async (
  ...
  const idFilter = excludedId
    ? ne(transactionTemplates.id, excludedId)
    : undefined;
  const row = await db
...
const readTemplateInTransaction = async (
  ...
  transactionDb.getFirstAsync<TransactionTemplate>(
    `SELECT
    id,
    name,
...
) => asserts current is CompleteScheduledTemplate = (
  selected,
  current,
  requireActive,
) => {
...
const backfillTemplateInTransaction = async (
  ...
  const inserted = await insertOccurrenceRows(
    transactionDb,
    current,
    dates,
    now,
...
export async function getNextAvailableTemplateName(
  base: string,
  excludedId?: string,
): Promise<string> {
  ...
  try {
    const rows = await db
      ...
      .where(
        and(
          isNull(transactionTemplates.deletedAt),
          excludedId ? ne(transactionTemplates.id, excludedId) : undefined,
        ),
      )
      .all();
    return nextAvailableTemplateName(
      base,
      rows.map((row) => row.name),
    );
  } catch (error) {
    failure("next_available_name", "query", error, {
...
export async function listTemplates(
  ...
  try {
    ...
    for (const row of activeTransactions) {
      ...
      latestUse.set(
        row.templateId,
        Math.max(latestUse.get(row.templateId) ?? 0, row.transactionDate),
      );
    }
        ...
        if (
          ...
          (!template.category ||
            !normalizedCategories.includes(
              normalizeTemplateText(template.category),
            ))
        )
          return false;
        if (!search) return true;
        return [template.name, template.description, template.category].some(
          (value) =>
            value !== null && normalizeTemplateText(value).includes(search),
        );
      })
      .sort((left, right) => {
        ...
        return normalizeTemplateText(left.name).localeCompare(
          normalizeTemplateText(right.name),
        );
      });
  } catch (error) {
...
export async function createTemplate(
  ...
  try {
    ...
    const row: TransactionTemplate = {
      ...
      verified:
        normalized.verified === null ? null : normalized.verified ? 1 : 0,
      recurrenceValue: normalized.recurrenceValue,
      startDate: normalized.startDate,
      scheduleCursorAt: normalized.recurrenceValue
        ? normalized.startDate
        : null,
      scheduleActive:
        normalized.recurrenceValue && normalized.scheduleActive ? 1 : 0,
      deletedAt: null,
      createdAt: now,
      updatedAt: now,
    };
    ...
    const normalizedError = isUniqueNameError(error)
...
export async function updateTemplate(
  ...
  try {
    ...
    `;
    const scheduleFloor =
      normalized.startDate === null ? now : Math.max(now, normalized.startDate);
...
export async function convertTemplateToManual(
  id: string,
  now = Date.now(),
): Promise<TransactionTemplate | null> {
...
export async function quickAddTemplate(
  id: string,
  now = Date.now(),
): Promise<Transaction> {
  ...
  try {
    ...
    const transaction = await createTransaction(
      mapTemplateToTransaction(draft, now, id),
    );
...
export async function previewTemplateBackfill(
  draft: TemplateDraft,
  now = Date.now(),
): Promise<number> {
  const draftDetails = safeDraftDetails(draft);
...
export async function backfillTemplate(
  id: string,
  now = Date.now(),
): Promise<number> {
  ...
  try {
    ...
    let inserted = 0;
    await sqlite.withExclusiveTransactionAsync(async (transactionDb) => {
      inserted = await backfillTemplateInTransaction(
        transactionDb,
        template,
        now,
      );
  ...
  try {
    ...
    for (const template of scheduled) {
      try {
        let inserted = 0;
        await sqlite.withExclusiveTransactionAsync(async (transactionDb) => {
          inserted = await processScheduledTemplateInTransaction(
            transactionDb,
            template,
            now,
          );
