...
jest.mock("expo-sqlite", () => {
  ...
  (
    globalThis as { __templateRepositoryDatabase?: typeof database }
  ).__templateRepositoryDatabase = database;
...
const beforeNextExclusiveTransaction = (
...
const beforeNextTemplateUpdate = (
  callback: (database: {
...
describe("template repository", () => {
  ...
  it("enforces active normalized names and allows a deleted name to be reused", async () => {
    ...
    await expect(
      createTemplate(validDraft({ name: " coffee " })),
    ...
    await expect(
      createTemplate(validDraft({ name: "coffee" })),
    ).resolves.toEqual(expect.objectContaining({ name: "coffee" }));
  });
  ...
  it("suggests the next active-name suffix while excluding the edited template", async () => {
    ...
    await expect(getNextAvailableTemplateName(" coffee ")).resolves.toBe(
      "coffee 3",
    );
    await expect(
      getNextAvailableTemplateName("Coffee", first.id),
    ).resolves.toBe("Coffee");
  });

  it("filters by search, manual or scheduled type, and category", async () => {
    await createTemplate(
      validDraft({ name: "Morning Coffee", category: "Food" }),
    );
    await createTemplate(
      scheduledDraft({ name: "Monthly Rent", category: "Home" }),
    );
    await createTemplate(
      validDraft({
        name: "Bus pass",
    ...
    expect(
      (await listTemplates({ search: "coffee" })).map(({ name }) => name),
    ).toEqual(["Morning Coffee"]);
    expect(
      (await listTemplates({ type: "scheduled" })).map(({ name }) => name),
    ).toEqual(["Monthly Rent"]);
    expect(
      (await listTemplates({ type: "manual" })).map(({ name }) => name).sort(),
    ).toEqual(["Bus pass", "Morning Coffee"]);
    expect(
      (await listTemplates({ category: "Travel" })).map(({ name }) => name),
    ).toEqual(["Bus pass"]);
    expect(
      (await listTemplates({ category: "travel" })).map(({ name }) => name),
    ).toEqual(["Bus pass"]);
  ...
  it("sorts by linked use, updatedAt, createdAt, then normalized name", async () => {
    ...
    const olderFallback = await createTemplate(
      validDraft({ name: "Zulu" }),
      30,
    );
    const betaFallback = await createTemplate(validDraft({ name: "Beta" }), 40);
    const alphaFallback = await createTemplate(
      validDraft({ name: "Alpha" }),
      40,
    );
    await db
      ...
      .set({ updatedAt: 50 })
      .where(
        sql`${transactionTemplates.id} IN (${olderFallback.id}, ${betaFallback.id}, ${alphaFallback.id})`,
      )
      .run();
    await db
      .insert(transactions)
  ...
  it("uses defaults and source linkage for Quick Add without moving a scheduled cursor", async () => {
    ...
    await expect(quickAddTemplate(template.id, 123)).resolves.toEqual(
  ...
  it("rejects Quick Add when a manual template is incomplete", async () => {
    const partial = await createTemplate(
      validDraft({ amount: null, description: null, category: "Food" }),
    );
    await expect(quickAddTemplate(partial.id, 123)).rejects.toThrow(
      "Quick Add requires a positive amount and description",
    );
  });
  ...
  it("rejects resuming a manual template without mutating it", async () => {
    ...
    await expect(resumeTemplate(template.id, 70)).rejects.toThrow(
      "Only complete scheduled templates can be resumed",
    );
  ...
  it("rejects resuming an incomplete migrated schedule without mutating it", async () => {
    ...
    const before = await db
      .select()
      .from(transactionTemplates)
      .where(eq(transactionTemplates.id, "migrated-incomplete"))
      .get();

    await expect(resumeTemplate("migrated-incomplete", 70)).rejects.toThrow(
      "Only complete scheduled templates can be resumed",
    );
  ...
  it("rejects resuming a whitespace-recurrence preserved row without mutating it", async () => {
    await db
      .insert(transactionTemplates)
      .values({
        id: "whitespace-recurrence",
        name: "Whitespace recurrence",
        normalizedName: "whitespace recurrence",
        amount: 10,
        transactionType: "expense",
    ...
    const before = await db
      .select()
      .from(transactionTemplates)
      .where(eq(transactionTemplates.id, "whitespace-recurrence"))
      .get();

    await expect(resumeTemplate("whitespace-recurrence", 70)).rejects.toThrow(
      "Only complete scheduled templates can be resumed",
    );
  ...
  it("keeps a future start as the hard floor when resuming", async () => {
    ...
    await expect(resumeTemplate(template.id, 70)).resolves.toEqual(
      expect.objectContaining({
        scheduleActive: 1,
  ...
  it("updates updatedAt when pausing", async () => {
    ...
    await expect(pauseTemplate(template.id, 50)).resolves.toEqual(
      expect.objectContaining({
        scheduleActive: 0,
        updatedAt: 50,
      }),
    );
    expect(await getTemplate(template.id)).toEqual(
      expect.objectContaining({
        scheduleActive: 0,
        updatedAt: 50,
      }),
  ...
  it("keeps paused schedules available manually, resumes from now, and converts to manual", async () => {
    ...
    await pauseTemplate(template.id, 50);
    await expect(
      processScheduledTemplates(new Date("2026-04-15T00:00:00.000Z")),
    ).resolves.toEqual([]);
    await expect(quickAddTemplate(template.id, 60)).resolves.toEqual(
  ...
  it("preserves a cursor for ordinary edits and resets it for cron or start edits", async () => {
    ...
    await updateTemplate(
      template.id,
      scheduledDraft({ name: "Updated rent" }),
      50,
    );
    expect((await getTemplate(template.id))?.scheduleCursorAt).toBe(
      template.startDate,
    );

    await updateTemplate(
      template.id,
      scheduledDraft({ name: "Updated rent", recurrenceValue: "0 0 2 * *" }),
      60,
    );
    expect((await getTemplate(template.id))?.scheduleCursorAt).toBe(
      template.startDate,
  ...
  it("preserves an existing cursor later than now and start when resuming", async () => {
    ...
    await expect(
      resumeTemplate(template.id, template.startDate! + 10_000),
    ).resolves.toEqual(
      expect.objectContaining({
        scheduleCursorAt: laterCursor,
        scheduleActive: 1,
      }),
    );
  });
  ...
  it("keeps incomplete paused schedule rows representable and out of automatic processing", async () => {
    ...
    await db
      ...
      .values([
        ...
        },
        {
          ...
          normalizedName: "missing description",
          amount: 3,
          description: null,
          startDate: 3,
          scheduleCursorAt: 3,
    ...
    await expect(
      processScheduledTemplates(new Date(10000000000)),
    ).resolves.toEqual([]);
  ...
  it("does not let a stale inactive-to-active edit overwrite a concurrent manual conversion", async () => {
    ...
    await expect(
      updateTemplate(template.id, scheduledDraft({ scheduleActive: true }), 40),
    ).resolves.toBeNull();
    expect(
      await db
        .select()
  ...
  it("does not let a stale schedule edit overwrite a concurrent schedule edit", async () => {
    ...
    await expect(
      updateTemplate(
        template.id,
        scheduledDraft({ recurrenceValue: "0 0 2 * *" }),
        40,
      ),
    ).resolves.toBeNull();
    expect(
  ...
  it.each([
    ...
    },
    {
      ...
      mutate: async (id: string) => {
        await db
          .update(transactionTemplates)
          .set({ deletedAt: 101, updatedAt: 101 })
          .where(eq(transactionTemplates.id, id))
          .run();
      },
    ...
    },
    {
      ...
      mutate: async (id: string) => {
        await db
          .update(transactionTemplates)
          .set({ recurrenceValue: "not a cron", updatedAt: 102 })
          .where(eq(transactionTemplates.id, id))
          .run();
      },
    ...
    "re-reads after a concurrent $name before resume and never activates invalid state",
  ...
  it("requires exactly one guarded row update when resuming", async () => {
    ...
    await expect(resumeTemplate(template.id, 70)).rejects.toThrow(
      "Scheduled template changed while resuming",
    );
  ...
  it.each([
    ...
    },
    {
      ...
      mutate: async (id: string) => {
        await db
          .update(transactionTemplates)
          .set({ deletedAt: 222, updatedAt: 222 })
          .where(eq(transactionTemplates.id, id))
          .run();
      },
    ...
    "rejects a stale scheduler snapshot after $name before transaction entry",
    async ({ mutate, expected }) => {
      const template = await createTemplate(scheduledDraft());
      beforeNextExclusiveTransaction(() => mutate(template.id));
  ...
  it("uses deterministic occurrence ids so scheduler retries insert no duplicates", async () => {
    ...
    await expect(processScheduledTemplates(now)).resolves.toEqual([
      { id: template.id, incurred: 3 },
    ]);
    await db
      .update(transactionTemplates)
      .set({ scheduleCursorAt: template.startDate })
      .where(eq(transactionTemplates.id, template.id))
      .run();
    await expect(processScheduledTemplates(now)).resolves.toEqual([
      { id: template.id, incurred: 0 },
    ]);
    expect(
      await db
        .select()
        .from(transactions)
        .where(eq(transactions.templateId, template.id))
        .all(),
    ).toHaveLength(3);
  });
  ...
  it("previews backfill count and inserts occurrences with the cursor atomically", async () => {
    ...
    expect((await getTemplate(template.id))?.scheduleCursorAt).toBe(
      Math.max(...inserted.map(({ transactionDate }) => transactionDate)),
    );
  });
  ...
  it("backfills a complete inactive schedule and preserves its paused state", async () => {
    const now = Date.parse("2026-04-15T00:00:00.000Z");
    const template = await createTemplate(
      scheduledDraft({ scheduleActive: false }),
    );
    ...
    expect(await getTemplate(template.id)).toEqual(
      expect.objectContaining({
        scheduleActive: 0,
        scheduleCursorAt: Math.max(
          ...inserted.map(({ transactionDate }) => transactionDate),
        ),
      }),
    );
  });
  ...
  it.each([
    ...
    },
    {
      ...
      mutate: async (id: string) => {
        await db
          .update(transactionTemplates)
          .set({ deletedAt: 622, updatedAt: 622 })
          .where(eq(transactionTemplates.id, id))
          .run();
      },
    ...
    "rejects a stale backfill snapshot after $name before transaction entry",
    async ({ mutate, expected }) => {
      const template = await createTemplate(scheduledDraft());
      beforeNextExclusiveTransaction(() => mutate(template.id));

      await expect(
        backfillTemplate(template.id, Date.parse("2026-04-15T00:00:00.000Z")),
      ).rejects.toThrow();
  ...
  it("rolls back backfill occurrences when the guarded cursor update changes no rows", async () => {
    ...
    await expect(
      ...
      "Scheduled template changed while committing occurrences",
    );
  ...
  it("never logs template names, search text, category values, or full filters", async () => {
    ...
    expect(infoSpy).toHaveBeenCalledWith("[templates.list][stage=query]", {
      has_search: true,
      type: "manual",
      category_count: 1,
    });
      ...
      .values([
        {
          id: "tea-1",
          amount: -3,
          transactionDate: Date.parse("2026-06-01T00:00:00.000Z"),
          description: "Tea",
          category: "Food",
          templateId: null,
          verified: 0,
        ...
        },
        {
          id: "tea-2",
          amount: -3,
          transactionDate: Date.parse("2026-07-01T00:00:00.000Z"),
          description: "Tea",
          category: "Food",
          templateId: null,
          verified: 0,
