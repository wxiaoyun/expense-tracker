...
describe('v2 database schema', () => {
  ...
  it('creates required tables, columns, and indexes', () => {
    ...
    expect(tables).toEqual(['categories', 'recurring_transactions', 'settings', 'transactions']);
  ...
  it('preserves 2,246 legacy rows including duplicate recurrence dates', () => {
    const database = new DatabaseSync(':memory:');
    database.exec(DATABASE_SCHEMA_SQL);
    const insert = database.prepare('INSERT INTO transactions (id, amount, transaction_date, description, category, recurring_transaction_id, verified, notes, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    database.exec('BEGIN');
    for (let index = 1; index <= 2246; index += 1) {
      insert.run(`legacy-${index}`, -index, index <= 2 ? 1776528000000 : index, `Expense ${index}`, 'Other', index <= 2 ? 'legacy-rule-1' : null, 0, null, index, index);
    }
    database.exec('COMMIT');
    expect(database.prepare('SELECT count(*) AS count FROM transactions').get()).toEqual({ count: 2246 });
    expect(database.prepare('SELECT count(*) AS count FROM transactions WHERE recurring_transaction_id = ? AND transaction_date = ?').get('legacy-rule-1', 1776528000000)).toEqual({ count: 2 });
    database.close();
  });
});
