...
describe("latest database schema", () => {
  ...
  it("declares the same template checks in the Drizzle schema", () => {
    expect(
      getTableConfig(transactionTemplates)
  ...
  it("enforces template invariants while allowing incomplete paused legacy schedules", () => {
    ...
    const run = (value: ReturnType<typeof row>) =>
      insert.run(
        value.id,
        value.id,
        value.id,
    ...
    expect(() => run(row({ id: "zero", amount: 0 }))).toThrow(
      /chk_templates_amount/,
    );
    expect(() =>
      run(row({ id: "infinite", amount: Number.POSITIVE_INFINITY })),
    ).toThrow(/chk_templates_amount/);
    expect(() =>
      run(
        row({
          id: "empty",
    ...
    expect(
      database
        .prepare(
          "SELECT schedule_active FROM transaction_templates WHERE id = ?",
        )
        .get("paused-incomplete"),
    ).toEqual({ schedule_active: 0 });
    database.close();
  });
  ...
  it("preserves 2,246 imported rows including duplicate recurrence dates", () => {
    ...
    database.exec(DATABASE_SCHEMA_SQL);
    const insert = database.prepare(
      "INSERT INTO transactions (id, amount, transaction_date, description, category, template_id, verified, notes, deleted_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    );
    database.exec("BEGIN");
    for (let index = 1; index <= 2246; index += 1) {
      insert.run(
        `legacy-${index}`,
        -index,
        index <= 2 ? 1776528000000 : index,
        `Expense ${index}`,
        "Other",
        index <= 2 ? "legacy-rule-1" : null,
        0,
        null,
        null,
        index,
    ...
    expect(
      database
        .prepare(
          "SELECT count(*) AS count FROM transactions WHERE template_id = ? AND transaction_date = ?",
        )
        .get("legacy-rule-1", 1776528000000),
    ).toEqual({ count: 2 });
    database.close();
  });
});
