...
describe("template core", () => {
  it("normalizes names without changing display text", () => {
    expect(normalizeTemplateText(" Weekly   Groceries ")).toBe(
      "weekly groceries",
    );
  });
  ...
  it("calculates deterministic calendar lookback cutoffs", () => {
    ...
    expect(suggestionLookbackStart("3m", now)).toBe(
      new Date(2026, 4, 31, 12).getTime(),
    );
    expect(suggestionLookbackStart("6m", now)).toBe(
  ...
  it("requires one reusable field and validates scheduled templates on submit", () => {
    ...
    expect(
      validateTemplateDraft(
        draft({ recurrenceValue: "0 0 1 * *", startDate: 1 }),
      ),
    ).toEqual({
      ok: false,
      field: "amount",
  ...
  it("rejects non-finite and non-positive amounts even when another reusable field exists", () => {
    expect(
      validateTemplateDraft(draft({ description: "Lunch", amount: NaN })),
    ).toEqual({
      ok: false,
      field: "amount",
      message: "Amount must be greater than zero",
    ...
    expect(
      validateTemplateDraft(draft({ category: "Food", amount: Infinity })),
    ).toEqual({
      ok: false,
      field: "amount",
      message: "Amount must be greater than zero",
  ...
  it("counts transaction type as a reusable field", () => {
    expect(validateTemplateDraft(draft({ transactionType: "income" }))).toEqual(
      { ok: true },
    );
  });

  it("accepts unscheduled drafts with non-amount reusable fields", () => {
    expect(validateTemplateDraft(draft({ description: "Lunch" }))).toEqual({
      ok: true,
    });
    expect(validateTemplateDraft(draft({ category: "Food" }))).toEqual({
      ok: true,
    });
  });
  ...
  it("suppresses active-template duplicates using default category and type", () => {
    ...
    expect(
      buildTemplateSuggestions(rows, [draft({ description: "Coffee" })]),
    ).toEqual([]);
  });
  ...
  it("suggests a suffix and chooses focus from creation context", () => {
    expect(nextAvailableTemplateName("Coffee", ["coffee", "Coffee 2"])).toBe(
      "Coffee 3",
    );
    expect(
      getTransactionInitialFocus({
