...
describe("latest schema migration", () => {
  ...
  it("atomically migrates V2 recurring rules and linked transactions", () => {
    ...
    expect(
      database
        .prepare(
          "SELECT id, amount, transaction_type, schedule_cursor_at FROM transaction_templates ORDER BY id",
        )
        .all(),
    ).toEqual([
      {
        ...
        schedule_cursor_at: 150,
      },
      {
        id: "rule-2",
        amount: 200,
        transaction_type: "income",
        schedule_cursor_at: 200,
      },
    ]);
    expect(
      database
        .prepare("SELECT template_id FROM transactions ORDER BY id")
        .all(),
    ).toEqual([{ template_id: "rule-1" }, { template_id: "rule-2" }]);
    expect(
      database
        .prepare(
          "SELECT notes, deleted_at, source FROM transactions ORDER BY id",
        )
        .all(),
    ).toEqual([
  ...
  it("preserves whitespace-only and zero-valued V2 rules with every link", () => {
    ...
    expect(
      database
        .prepare("SELECT id, template_id FROM transactions ORDER BY id")
        .all(),
    ).toEqual([
  ...
  it("pauses invalid cron, start, cursor, and nonfinite V2 rules without dropping rows", () => {
    ...
    `,
      ...
      },
      {
        ...
        recurrence_value: "not a cron",
        start_date: 100,
        schedule_cursor_at: 100,
        schedule_active: 0,
      },
      {
        id: "bad-cursor",
  ...
  it("rolls back every schema change when a mapped insert fails", () => {
    ...
    const errorLog = jest
      .spyOn(console, "error")
      .mockImplementation(() => undefined);
    const adapter = new ExpoSQLiteSyncAdapter(database, (sql) =>
      sql.includes("INSERT INTO transaction_templates"),
    );

    expect(() => runSchemaMigrations(asExpoDatabase(adapter))).toThrow(
    ...
    expect(
      database
        .prepare("SELECT count(*) AS count FROM recurring_transactions")
        .get(),
    ).toEqual({ count: 2 });
    expect(
      database
        .prepare(
          "SELECT recurring_transaction_id FROM transactions ORDER BY id",
        )
        .all(),
    ).toEqual([
    ...
    expect(
      database
        .prepare(
          "SELECT name FROM sqlite_master WHERE type='table' AND name='transactions_v3'",
        )
        .get(),
    ).toBeUndefined();
    expect(errorLog).toHaveBeenCalledWith(
