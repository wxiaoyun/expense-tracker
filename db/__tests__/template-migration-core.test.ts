...
describe("template migration core", () => {
  ...
  it("uses preserved ids for deterministic nonempty fallback names", () => {
    expect(
      ...
      ]).map(({ id, name, normalizedName, description }) => ({
        id,
        name,
        normalizedName,
        description,
      ...
      {
        id: "rule-a",
        name: "Template rule-a",
        normalizedName: "template rule-a",
        description: "   ",
      },
      {
        id: "rule-b",
        name: "Template rule-b",
        normalizedName: "template rule-b",
        description: " \t ",
      },
    ]);
  });
  ...
  it("pauses every incomplete or invalid source schedule while preserving its fields", () => {
    ...
    expect(
      rows.map(({ id, scheduleActive }) => ({ id, scheduleActive })),
    ).toEqual([
    ...
    expect(rows.find(({ id }) => id === "amount")).toEqual(
      expect.objectContaining({
        amount: null,
        description: "Amount",
      }),
    );
    expect(rows.find(({ id }) => id === "cron")?.recurrenceValue).toBe(
      "not a cron",
    );
