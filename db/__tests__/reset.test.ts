...
describe("reset all data", () => {
  ...
  it("physically clears active and soft-deleted transactions and templates before categories and settings", async () => {
    const rows = new Map<
      unknown,
      Array<{ id: string; deletedAt?: number | null }>
    >([
      [
        transactions,
        [
          { id: "active-tx", deletedAt: null },
          { id: "deleted-tx", deletedAt: 10 },
        ],
      ],
      [
        transactionTemplates,
        [
          { id: "active-template", deletedAt: null },
          { id: "deleted-template", deletedAt: 20 },
        ],
      ],
    ...
    expect(
      [...rows.values()].every((tableRows) => tableRows.length === 0),
    ).toBe(true);
  });
  ...
  it("reports the table stage when a partial reset fails", async () => {
    ...
    expect(error).toHaveBeenCalledWith("[db.reset] reset partially failed", {
      stage: "templates",
      error: "Error: forced delete failure",
    });
  });
});
