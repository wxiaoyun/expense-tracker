...
describe("quick database validation", () => {
  ...
  it("requires integrity, four required tables, exact latest columns, and user_version 3", async () => {
    ...
    await expect(
      quickValidateDatabase("/private/secret-database.db"),
    ).resolves.toBe(true);

    expect(info).toHaveBeenCalledWith(
      "[db.quick_validate][stage=result] validation completed",
      {
        integrity_ok: true,
  ...
  it.each([
    ...
    [
      "old transaction columns",
      {
        transactionColumns: V3_REQUIRED_COLUMNS.transactions.filter(
    ...
    [
      ...
      {
        templateColumns: V3_REQUIRED_COLUMNS.transaction_templates.filter(
          (name) => name !== "deleted_at",
        ),
      },
    ],
  ] satisfies Array<[string, QuickFixtureOptions]>)(
    "rejects %s",
    async (_label, options) => {
      mockOpenDatabaseSync.mockReturnValue(createFixture(options));
      jest.spyOn(console, "info").mockImplementation(() => undefined);

      await expect(quickValidateDatabase("/not-logged.db")).resolves.toBe(
        false,
      );
    },
  );
  ...
  it("preserves fallback open behavior without logging the database path", async () => {
    ...
    mockOpenDatabaseSync
      .mockImplementationOnce(() => {
        throw new Error("primary open failure");
      })
    ...
    await expect(quickValidateDatabase("/private/fallback.db")).resolves.toBe(
      true,
    );
  ...
  it("reports a final fallback-open failure without exposing its path", async () => {
    ...
    await expect(quickValidateDatabase("/private/unopenable.db")).resolves.toBe(
      false,
    );
    ...
    ).not.toContain("/private/unopenable.db");
  });
  ...
  it("logs concise structured final and close failures without dumps or paths", async () => {
    ...
    await expect(quickValidateDatabase("/private/no-log.db")).resolves.toBe(
      true,
    );
