...
describe("recurring compatibility mapping", () => {
  ...
  it("rejects zero, non-finite, and non-numeric compatibility amounts", () => {
    ...
    expect(() => mapRecurringToScheduledTemplate(recurring, [])).toThrow(
      "Amount must be greater than zero",
    );
    expect(() =>
      mapRecurringToScheduledTemplate({ ...recurring, amount: Number.NaN }, []),
    ).toThrow("Amount must be greater than zero");
    expect(() =>
      mapRecurringToScheduledTemplate(
        { ...recurring, amount: Number.POSITIVE_INFINITY },
        [],
      ),
    ).toThrow("Amount must be greater than zero");
    expect(() =>
      mapRecurringToScheduledTemplate(
        { ...recurring, amount: Number.NEGATIVE_INFINITY },
        [],
      ),
    ).toThrow("Amount must be greater than zero");
  });
