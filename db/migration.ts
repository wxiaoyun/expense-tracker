...
const migrationRecoveryName = () =>
  `expense_tracker_migration_recovery_${Date.now()}.db`;
const migrationRecoveryPathFor = (name: string) =>
  `${SQLite.defaultDatabaseDirectory}/${name}`;
 ...
 */
export const seedPresetCategories = async (
  ...
  console.info(
    "[migration][stage=seed_categories] inserting preset categories",
    { count: values.length },
  );
  await newDb.insert(categories).values(values).onConflictDoNothing().run();
  console.info("[migration][stage=seed_categories] preset categories ready", {
    count: values.length,
  });
};
 ...
 */
export const isMigrationDone = async (
  newDb: ReturnType<typeof drizzle>,
): Promise<boolean> => {
...
export const markMigrationComplete = async (
  ...
  await newDb
    .insert(settings)
    .values({ key: "app.migrated", value: "1" })
    .onConflictDoUpdate({ target: settings.key, set: { value: "1" } })
    .run();
  await newDb
    .insert(settings)
    .values({ key: "app.migrated_at", value: String(now) })
...
export const legacyDbExists = (): boolean => {
  try {
    ...
    const tables = legacyDb.getAllSync(
      "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('transactions', 'recurring_transactions')",
    );
    const transactionColumns = legacyDb.getAllSync<{
      name: string;
      type: string;
    ...
    console.info(
      "[migration][stage=detect_legacy][reason=not_found] legacy database unavailable",
      {
        error: String(error),
      },
    );
...
export const getLegacyCounts = async (): Promise<{
  ...
  try {
    const legacyDb = openLegacyDatabase();
    const txCount = legacyDb.getFirstSync<{ count: number }>(
      "SELECT COUNT(*) as count FROM transactions",
    );
    const recCount = legacyDb.getFirstSync<{ count: number }>(
    ...
    console.error(
      "[migration][stage=count_legacy] failed to count legacy rows",
      { error: String(error) },
    );
    return null;
  }
 ...
 */
export const backupLegacyDb = async (
  legacyDb: SQLite.SQLiteDatabase,
): Promise<string | null> => {
  try {
    const backupPath = await backupLegacyDatabase(
      async ({ to }) => {
        ...
        try {
          ...
          const integrity = destinationDb.getFirstSync<{
            integrity_check: string;
          }>("PRAGMA integrity_check");
          if (integrity?.integrity_check !== "ok")
            throw new Error("Legacy backup integrity check failed");
        } finally {
    ...
    console.info("[migration][stage=backup_legacy] legacy database backed up", {
      backupPath,
    });
    return backupPath;
  } catch (error) {
    console.error(
      "[migration][stage=backup_legacy] legacy database backup failed",
      { error: String(error) },
    );
    return null;
  }
...
export const runMigration = async (
  ...
  try {
    ...
    console.info(
      "[migration][stage=copy_transactions] transaction copy finished",
      { count: txMigrated },
    );

    const recMigrated = await migrateRecurringTransactions(legacyDb, newDb);
    console.info("[migration][stage=copy_templates] template copy finished", {
      count: recMigrated,
    });

    const newTransactionCount = await newDb
      .select({ count: sql<number>`count(*)` })
      .from(transactions)
      .get();
    const newTemplateCount = await newDb
      .select({ count: sql<number>`count(*)` })
    ...
    const newRelationshipCount = await newDb
      ...
      .innerJoin(
        transactionTemplates,
        eq(transactions.templateId, transactionTemplates.id),
      )
      .get();
    if (
      Number(newTransactionCount?.count ?? 0) !== counts.transactions ||
      ...
      console.error(
        "[migration][stage=verify_counts] migrated counts or relationships do not match source",
        {
      ...
      return {
        success: false,
        error:
          "Migrated row counts or relationships do not match source database",
      };
    }
    console.info(
      "[migration][stage=verify_counts] migrated counts and relationships match source",
      counts,
    );
    ...
    if (!backupPath) {
      return {
        success: false,
        error: "Legacy database backup failed. Migration remains retryable.",
      };
    }
    ...
    console.error("[migration][stage=run] migration failed", {
      error: String(error),
    });
    ...
    if (legacyDb) {
      try {
        ...
        console.error(
          "[migration][stage=close_legacy] legacy database close failed",
