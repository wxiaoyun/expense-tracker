...
export function ExpenseFilterBar({
  ...
  const { input, onPrimary, placeholder, primary, secondaryText, text } =
    useThemeColors();
        ...
        {PRESETS.map((item) => {
          ...
          return (
            ...
            >
              <Text
                style={{
                  color: selected ? onPrimary : secondaryText,
                  fontWeight: "600",
                }}
              >
                {item.label}
          ...
          <View style={{ flex: 1, alignItems: "flex-start" }}>
            <CompactDatePicker
              ...
              onValueChange={(date) => {
                if (
                  !onCustomStartChange ||
                  date.getTime() === customStart?.getTime()
                )
                  return;
                selectionFeedback();
          ...
          <View style={{ flex: 1, alignItems: "flex-start" }}>
            <CompactDatePicker
              ...
              onValueChange={(date) => {
                if (
                  !onCustomEndChange ||
                  date.getTime() === customEnd?.getTime()
                )
                  return;
                selectionFeedback();
