...
jest.mock("@/hooks/useTransactionsQuery", () => ({
  ...
  useInfiniteTransactionListQuery: (...args: unknown[]) =>
    mockUseInfiniteTransactionListQuery(...args),
}));

jest.mock("@/hooks/useQueryClient", () => ({
  useInvalidateTransactions: () => mockInvalidateTransactions,
  useInvalidateTransactionsAndTemplates: () =>
    mockInvalidateTransactionsAndTemplates,
}));
...
jest.mock("@/hooks/useFilter", () => ({
  ...
  useDateRange: () => [
    { preset: "all", start: new Date(0), end: new Date(1) },
    jest.fn(),
  ],
  useSearch: () => ["", jest.fn()],
}));
...
jest.mock("@/db/transaction", () => ({
  ...
  softDeleteTransaction: (...args: unknown[]) =>
    mockSoftDeleteTransaction(...args),
}));
...
describe("TransactionRow", () => {
  it("edits from the card body while verification and menu remain sibling press targets", async () => {
    ...
    await fireEvent.press(
      screen.getByRole("checkbox", { name: "Mark Coffee verified" }),
    );
    await fireEvent.press(
      screen.getByRole("button", { name: "Actions for Coffee" }),
    );
...
describe("HomeScreen transaction action menu integration", () => {
  ...
  it("routes Save as Template and View Template with object pathname params from active template availability", async () => {
    ...
    await fireEvent.press(
      activeScreen.getByTestId("menu-action-view-template"),
    );
    ...
    await fireEvent.press(
      missingTemplateScreen.getByTestId("menu-action-save-template"),
    );
    expect(mockPush).toHaveBeenCalledWith({
      pathname: "/(drawer)/template-edit",
  ...
  it("logs template load failures and renders transactions conservatively with Save as Template", async () => {
    ...
    mockUseQuery.mockImplementation(
      ({ queryKey }: { queryKey: readonly unknown[] }) => {
        if (queryKey[0] === "templates")
          return { data: [template()], error: templateError };
        return { data: [], error: null };
      },
    );
  ...
  it("uses structured stages for verification and edit actions", async () => {
    ...
    await waitFor(() =>
      expect(mockSetVerification).toHaveBeenCalledWith("txn-1", 1),
    );
  ...
  it("confirms then soft deletes, logging structured stages and invalidating after success", async () => {
    ...
    expect(infoSpy.mock.invocationCallOrder[0]).toBeLessThan(
      mockSoftDeleteTransaction.mock.invocationCallOrder[0],
    );
    expect(mockInvalidateTransactionsAndTemplates).toHaveBeenCalledTimes(1);
    expect(mockSoftDeleteTransaction.mock.invocationCallOrder[0]).toBeLessThan(
      mockInvalidateTransactionsAndTemplates.mock.invocationCallOrder[0],
    );
