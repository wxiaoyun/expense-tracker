...
const columnsFrom = (database: DatabaseSync) =>
  Object.fromEntries(
    database
      .prepare(
...
const createLatestDatabase = (label: string) => {
  ...
  database
    .prepare(
      "INSERT INTO transaction_templates VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    )
    .run(
      `${label}-template`,
      `${label} template`,
...
const createEmptyDatabase = () =>
  new ExpoSQLiteSyncAdapter(
    new DatabaseSync(":memory:") as SerializableDatabaseSync,
  );

const copyDatabase = jest.fn(
  async (source: ExpoSQLiteSyncAdapter, destination: ExpoSQLiteSyncAdapter) => {
    destination.database.deserialize(source.database.serialize());
  },
);

const validateIntegrity = (database: ExpoSQLiteSyncAdapter) => {
  const result = database.database.prepare("PRAGMA integrity_check").get() as {
...
const validateLatest = (database: ExpoSQLiteSyncAdapter) => {
  if (detectDatabaseVersion(database.database) !== 4)
    throw new Error("destination is not exact latest");
};
...
describe("recovery snapshot lifecycle", () => {
  const createHooks = () => {
    ...
    return {
      ...
      recovery,
      hooks: {
        removeStaleRecovery: jest.fn(async () => {
          calls.push("remove_stale");
        }),
        openRecovery: jest.fn(() => {
          calls.push("open");
          return recovery;
        }),
        closeRecovery: jest.fn(async () => {
          calls.push("close");
        }),
        deleteRecovery: jest.fn(async () => {
          calls.push("delete");
        }),
      },
    };
...
describe("result-based migration recovery", () => {
  ...
  it("restores the original live target when a migration returns failure after writes", async () => {
    ...
    expect(
      destination.database.prepare("SELECT id FROM transactions").all(),
    ).toEqual([{ id: "original-tx" }]);
    expect(
      destination.database
        .prepare("SELECT value FROM settings WHERE key = 'partial.marker'")
        .get(),
    ).toBeUndefined();
  ...
  it("commits the migrated live target when the operation succeeds", async () => {
    ...
    expect(
      destination.database
        .prepare("SELECT value FROM settings WHERE key = 'app.migrated'")
        .get(),
    ).toEqual({ value: "1" });
...
describe("backup schema compatibility", () => {
  ...
  it("rejects missing, mixed, and extra columns even with a supported version", () => {
    expect(
      detectBackupSchemaVersion(
        {
          ...V3_REQUIRED_COLUMNS,
          transactions: V3_REQUIRED_COLUMNS.transactions.filter(
      ...
      detectBackupSchemaVersion(
        {
          ...
          transaction_templates: [
            ...V3_REQUIRED_COLUMNS.transaction_templates,
            "unexpected",
          ],
        },
        3,
  ...
  it("uses separate databases to restore the latest schema directly and validates the destination", async () => {
    ...
    source.database
      .prepare("UPDATE transaction_templates SET deleted_at = 456")
      .run();
    const destination = createLatestDatabase("original");
    const recovery = createEmptyDatabase();
    ...
    expect(
      destination.database
        .prepare("SELECT id, template_id, deleted_at FROM transactions")
        .all(),
    ).toEqual([
    ...
    expect(
      destination.database
        .prepare("SELECT id, deleted_at FROM transaction_templates")
        .all(),
    ).toEqual([{ id: "source-template", deleted_at: 456 }]);
  ...
  it("uses separate databases and real copy semantics before migrating V2 links", async () => {
    ...
    expect(
      destination.database
        .prepare("SELECT id, template_id, deleted_at FROM transactions")
        .all(),
    ).toEqual([{ id: "tx-1", template_id: "rule-1", deleted_at: null }]);
    expect(
      destination.database
        .prepare("SELECT id, deleted_at FROM transaction_templates")
        .all(),
    ).toEqual([{ id: "rule-1", deleted_at: null }]);
  ...
  it("restores the original live schema and data after V2 post-copy migration fails", async () => {
    ...
    expect(
      destination.database
        ...
        .all(),
    ).toEqual([{ id: "original-tx", description: "original" }]);
  ...
  it("rolls back a replacement copy that mutates the destination and then fails", async () => {
    ...
    const destination = createLatestDatabase("original");
    const recovery = createEmptyDatabase();
    const failingCopy = jest.fn(
      async (from: ExpoSQLiteSyncAdapter, to: ExpoSQLiteSyncAdapter) => {
  ...
  it("logs rollback stage and safely surfaces replacement and rollback failures", async () => {
    ...
    const failingCopy = jest.fn(
      async (from: ExpoSQLiteSyncAdapter, to: ExpoSQLiteSyncAdapter) => {
        if (from === recovery && to === destination)
