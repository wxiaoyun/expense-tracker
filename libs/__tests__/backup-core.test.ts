...
describe("backup core", () => {
  ...
  it("builds filesystem-safe names", () => {
    expect(backupFilename(new Date("2026-08-11T01:02:03.000Z"))).toBe(
      "expense-tracker-2026-08-11T01-02-03-000Z.db",
    );
  });
  it("validates SQLite headers", () => {
    expect(
      hasSqliteHeader(new TextEncoder().encode("SQLite format 3\0data")),
    ).toBe(true);
  ...
  it("rejects missing, extra, or reordered restore columns", () => {
    ...
    expect(hasExactColumns(["id", "amount", "extra"], ["id", "amount"])).toBe(
      false,
    );
  });
});
