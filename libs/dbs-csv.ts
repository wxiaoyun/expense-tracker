...
const REQUIRED_HEADER = ['Transaction Date', 'Description', 'Debit Amount', 'Credit Amount'];
...
export const dbsRowId = (rawLine: string) => uuidv5(`dbs:${rawLine}`, DBS_NAMESPACE);

export const isPayLahTopUp = (description: string) => description.startsWith(PAYLAH_TOP_UP);

export const parseDbsCsv = (text: string): DbsParseResult => {
  const lines = text.replace(/^﻿/, '').split(/\r?\n/);
  const headerIndex = lines.findIndex((line) => line.startsWith('"Transaction Date"'));
  const header = headerIndex >= 0 ? splitCsvLine(lines[headerIndex]) : [];
  const col = Object.fromEntries(REQUIRED_HEADER.map((name) => [name, header.indexOf(name)]));
  ...
  for (const line of lines.slice(headerIndex + 1)) {
    ...
    const date = parse(fields[col['Transaction Date']] ?? '', 'dd MMM yyyy', new Date());
    ...
    const description = (fields[col['Description']] ?? '').trim() || (fields[codeCol] ?? '').trim();
    const amount = credit - debit;
    if (Number.isNaN(date.getTime()) || !Number.isFinite(amount) || amount === 0 || !description) {
      unreadable++;
      continue;
    }
    rows.push({ id: dbsRowId(line), transactionDate: date.getTime(), description, amount, notes: line });
  }
